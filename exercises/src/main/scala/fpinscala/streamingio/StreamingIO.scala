...
object SimpleStreamTransducers {
  ...
  object Process {
    ...
    def zip[I, O1, O2](
      p1: Process[I, O1],
      p2: Process[I, O2]): Process[I, (O1, O2)] =
      (p1, p2) match {
        case (Halt(), _) => Halt()
        case (_, Halt()) => Halt()
    ...
    def convertToCelsius(
      in: java.io.File,
      out: java.io.File): iomonad.Free[Par, Unit] = {
      def process: Process[String, String] =
        filter((l: String) => !l.startsWith("#")) |> filter(_.trim.nonEmpty) |> lift(
          l => toCelsius(l.toDouble).toString)
